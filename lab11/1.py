...
def query_data_by_pattern():
    ...
    if pattern:
        cur.execute(
            "SELECT * FROM phonebook WHERE name LIKE %s OR number LIKE %s",
            ('%' + pattern + '%', '%' + pattern + '%')
        )
        rows = cur.fetchall()
        if rows:
...
def insert_or_update_user():
    ...
    if name and number:
        cur.execute(
            "INSERT INTO phonebook(name, number) VALUES (%s, %s) ON CONFLICT (name) DO UPDATE SET number = excluded.number",
            (name, number)
        )
        conn.commit()
